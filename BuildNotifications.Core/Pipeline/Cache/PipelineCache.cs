...
internal class PipelineCache<T> : IPipelineCache<T>
{
    ...
    public bool Contains(CacheKey key) => _items.ContainsKey(key);

    public bool Contains(string providerId, string itemId) => Contains(new CacheKey(providerId, itemId));
    ...
    public bool ContainsValue(T value) => _items.ContainsValue(value);
    ...
    public IEnumerable<T> ContentCopy() => new List<T>(_items.Values);
    ...
    private readonly Dictionary<CacheKey, T> _items = new();
}